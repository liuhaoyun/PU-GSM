...
from Upsampling.generator import Generator, Twin
...
from Common.loss_utils import pc_distance,get_uniform_loss,get_repulsion_loss,twin_loss
...
class Model(object):
  ...
  def build_model(self):
      ...
      self.T = Twin(self.opts,self.is_training,name='Twin')

      # X -> Y
      self.G_y, self.G_pred_ori = self.G(self.input_x)
      self.F_in, self.F_out = self.T(self.G_y,self.input_y)
      self.dis_loss = self.opts.fidelity_w * pc_distance(self.G_y, self.input_y, dis_type='cd', radius=self.pc_radius)
      #self.emd_loss = self.opts.emd_w * pc_distance(self.G_y, self.input_y, dis_type='emd', radius=self.pc_radius)
      self.coarse_loss = self.opts.coarse_w * pc_distance(self.input_x, self.G_pred_ori, dis_type='cd', radius=self.pc_radius)
      self.twin_loss = self.opts.twin_w * twin_loss (self.F_in,self.F_out)
      ...
      self.pu_loss = self.dis_loss + self.uniform_loss + self.twin_loss + tf.losses.get_regularization_loss(scope='generator') + self.coarse_loss
  ...
  def summary_all(self):
      ...
      add_scalar_summary('loss/twin_loss', self.twin_loss, collection='gen')
  ...
  def test(self):
      ...
      self.pred_pc, _ = Gen(self.inputs)
      for i in range(round(math.pow(self.opts.up_ratio, 1 / 4)) - 1):
          self.pred_pc, _ = Gen(self.pred_pc)
      ...
      print("average time: ",  total_time/27)
